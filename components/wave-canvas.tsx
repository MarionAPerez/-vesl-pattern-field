...
function toneToY(tone: Moment["tone"], height: number, padding: number = 60): number {
  const centerY = height / 2;
  const amplitude = (height - padding * 2) / 2;
  
  // Get tone valence: -1 to 1
  const valence = TONE_VALENCE[tone];
  
  // Map to Y position (negative Y = up on canvas)
  return centerY - valence * amplitude * 0.75;
}
